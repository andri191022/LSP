...
left outer join  [Tb_Kabupaten] Jb on Ja.Kode_Kabupaten = Jb.Kode_Kabupaten
 WHERE a.Kode_Jejaring = @ID";
        ...
        public static Tb_Jejaring_cstm GetByNPSNJKodeKK(string npsnj, string kode_kk)
        {
            IDBHelper context = new DBHelper();
            string sqlQuery = @"SELECT 
a.Kode_Jejaring
...
left outer join  [Tb_SMK] Ja on Ja.NPSN = a.NPSN
left outer join  [Tb_Kabupaten] Jb on Ja.Kode_Kabupaten = Jb.Kode_Kabupaten
where a.NPSN =@NPSNJ and a.Kode_KK_Terlisensi =@Kode_KK_Terlisensi
";
            context.AddParameter("@NPSNJ", npsnj);
            context.AddParameter("@Kode_KK_Terlisensi", kode_kk);
            context.CommandText = sqlQuery;
            context.CommandType = System.Data.CommandType.Text;
            return DBUtil.ExecuteMapper<Tb_Jejaring_cstm>(context, new Tb_Jejaring_cstm()).FirstOrDefault();
        }
